...
for (let element of childrens)if (element.firstElementChild != null) element.firstElementChild.classList.add(`feedback-form__${element.firstElementChild.getAttribute("name")}`);
else element.classList.add(`feedback-form__${element.getAttribute("type")}`);
...
const removePrintFormValues = (event)=>{
    ...
    } else alert("El correo no tiene formato adecuado");
};
//Evento input que dispara la funcion callback cada que se teclea
values.addEventListener("input", (0, _lodashThrottleDefault.default)(setFormValues, 500));
